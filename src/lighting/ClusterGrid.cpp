...
namespace ofxRenderToolkit
{
    ...
    void ClusterGrid::Init(const ProjInfo& _projInfo)
    {
        ...
        m_planesX = new Plane[NUM_PLANES_X];
        m_planesY = new Plane[NUM_PLANES_Y];
        m_planesZ = new Plane[NUM_PLANES_Z];
    ...
    {
        m_nearPlane = Plane(glm::vec3(0.0f, 0.0f, -m_projInfo.nearZ), glm::vec3(0.0f, 0.0f, 1.0f));
        m_farPlane = Plane(glm::vec3(0.0f, 0.0f, -m_projInfo.farZ), glm::vec3(0.0f, 0.0f, 1.0f));
        ...
        {
            m_planesZ[z] = Plane(normal, distance);
    ...
    void ClusterGrid::AddPointLightToCluster(uint16_t light_idx, int x, int y, int z)
    {
    ...
    void ClusterGrid::CullPointLights(const ofMatrix4x4& _viewMatrix, const std::vector<PointLight>& _pointLights)
    {
        // reset clusters
        m_numLightIndices = 0;
        ...
        float horizFov = GetHorizontalFov(m_projInfo.fov, m_projInfo.aspectRatio);
        ...
        {
            const PointLight& light = _pointLights[light_idx];
            glm::vec4 posVS = viewMat * light.position;
            glm::vec3 lightPosVS = glm::vec3(posVS.x, posVS.y, posVS.z);

            if (false == SphereInFrustum(m_frustumPlanes, lightPosVS, light.radius))
            {
            ...
            SphereProjectionResult result = ProjectSphere(lightPosVS, light.radius, focalLength, aspectRatio, &rectMin, &rectMax);
            ...
            if (SphereProjectionResult::FULL == result)
            {
                int x0 = 0;
    ...
    void ClusterGrid::BindLightIndexTextures(GLuint _indexTexUnit, GLuint _pointerTableTexUnit)
    {
    ...
    void ClusterGrid::SortLightIndexList()
    {
        ...
        RadixSort16<uint16_t>(m_lightSortKeys, m_tempLightSortKeys, m_lightIndices, m_tempLightIndices, m_numLightIndices);
